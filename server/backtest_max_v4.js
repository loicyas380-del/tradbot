...
async function fetchBatch(symbols, start, end, batchSize = 5) {
  const cache = {};
  for (let i = 0; i < symbols.length; i += batchSize) {
    const batch = symbols.slice(i, i + batchSize);
    const results = await Promise.allSettled(batch.map(s => yfChart(s, start, end)));
    for (let j = 0; j < batch.length; j++) {
      if (results[j].status === "fulfilled" && results[j].value.length > 0) {
        cache[batch[j]] = results[j].value;
      }
    }
  }
  return cache;
...
function analyze(ana, idx, regime) {
  ...
  const volMultiplier = atrRatio > 0.05 ? 0.5 : 1.0; // Réduit taille si volatile

  if (regime === "bear") {
    if (rsiVal < 28 && isBouncing && volumeOK) return { side: "LONG", entry: price, sl: price - slDist, tp: price + tpDist, size: 0.06 * volMultiplier };
    if (rsiVal > 72 && isFalling && volumeOK) return { side: "SHORT", entry: price, sl: price + slDist, tp: price - tpDist, size: 0.06 * volMultiplier };
    return null;
  }
  if (regime === "neutral") {
    if (rsiVal < 35 && isBouncing && volumeOK) return { side: "LONG", entry: price, sl: price - slDist, tp: price + tpDist, size: 0.10 * volMultiplier };
    if (rsiVal > 65 && isFalling && volumeOK) return { side: "SHORT", entry: price, sl: price + slDist, tp: price - tpDist, size: 0.10 * volMultiplier };
    return null;
  }
  if (rsiVal < 40 && isBouncing && volumeOK) return { side: "LONG", entry: price, sl: price - slDist, tp: price + tpDist, size: 0.15 * volMultiplier };
  if (rsiVal > 60 && isFalling && volumeOK) return { side: "SHORT", entry: price, sl: price + slDist, tp: price - tpDist, size: 0.15 * volMultiplier };
  return null;
}

function backtest(rawData, capital, regime) {
  if (!rawData || rawData.length < 30) return null;
  const ana = computeIndicators(rawData);
  let cash = capital, peak = capital, maxDD = 0;
  let wins = 0, losses = 0, totalTrades = 0;
  let cooldown = 0, positions = [];
  let baseSize = regime === "bear" ? 0.06 : regime === "neutral" ? 0.10 : 0.15;
  ...
  for (let i = 20; i < ana.len; i++) {
    ...
    if (positions.length === 0) {
      const signal = analyze(ana, i, regime);
      if (signal) {
  ...
  return { totalTrades, wins, losses, winRate: +winRate, pnlPct: +pnlPct, finalBalance: cash, maxDrawdown: +(maxDD * 100).toFixed(1) };
}
...
function sliceData(fullData, start, end) {
  return fullData.filter(d => d.date >= new Date(start) && d.date <= new Date(end));
}

async function main() {
  const N = ALL_ASSETS.length;
  console.log(`\n═══════════════════════════════════════════════════════════════`);
  console.log(`  BOT MAX V4 — ${N} ACTIFS + FILTRE MARCHÉ (OPTIMISÉ)`);
  console.log(`═══════════════════════════════════════════════════════════════\n`);
  ...
  const fullStart = new Date(tests[tests.length - 1].start);
  fullStart.setMonth(fullStart.getMonth() - 1);
  const fullStartStr = fullStart.toISOString().split('T')[0];
  const fullEndStr = tests[0].end;

  console.log(`  Chargement de ${N} actifs (${fullStartStr} → ${fullEndStr})...`);
  console.log(`  (5 requêtes en parallèle, ~${Math.ceil(N/5) * 0.2}s)\n`);

  const t0 = Date.now();
  const dataCache = await fetchBatch(ALL_ASSETS, fullStartStr, fullEndStr, 5);
  const loaded = Object.keys(dataCache).length;
  const elapsed = ((Date.now() - t0) / 1000).toFixed(1);
  console.log(`  ✅ ${loaded}/${N} actifs chargés en ${elapsed}s\n`);

  for (const test of tests) {
    console.log(`═══ TEST ${test.label}/10 (${test.start} → ${test.end}) ═══`);

    const btcFull = dataCache["BTC-USD"];
    const spFull = dataCache["^GSPC"];
    const btcRegime = getMarketRegime(btcFull, test.end);
    const spRegime = getMarketRegime(spFull, test.end);

    let finalRegime = "neutral";
    // Si BTC et S&P sont en désaccord → neutral (prudent)
    if (btcRegime === "bear" && spRegime === "bear") finalRegime = "bear";
    else if (btcRegime === "bull" && spRegime === "bull") finalRegime = "bull";
    // Si un seul est bear → neutral (pas de bear complet, mais pas confiant)
    else if (btcRegime === "bear" || spRegime === "bear") finalRegime = "neutral";
    // Sinon neutral

    const re = finalRegime === "bull" ? "🟢BULL" : finalRegime === "bear" ? "🔴BEAR" : "🟡NEUT";
    console.log(`  Marché: BTC=${btcRegime} S&P=${spRegime} → ${re}\n`);
    ...
    for (const sym of ALL_ASSETS) {
      const fullData = dataCache[sym];
      if (!fullData) continue;
      const data = sliceData(fullData, test.start, test.end);
      if (!data || data.length < 30) continue;

      const result = backtest(data, balance, finalRegime);
    ...
    console.log(`  ${emoji} ${capital}€ → ${balance.toFixed(2)}€ (${pnlPct >= 0 ? '+' : ''}${pnlPct}%) | ${totalTrades} trades | ${winRate}% WR | DD: ${maxDD}% | ${assetCount} actifs\n`);

    if (assetResults.length > 0) {
      const sorted = assetResults.sort((a, b) => b.pnl - a.pnl).slice(0, 5);
      for (const a of sorted) {
        const e = a.pnl >= 0 ? "🟢" : "🔴";
        console.log(`    ${e} ${a.sym}: ${a.trades}t ${a.wr}% WR ${a.pnl >= 0 ? '+' : ''}${a.pnl}%`);
      }
    }
    console.log("");
  }
  ...
  console.log("═══════════════════════════════════════════════════════════════");
  console.log(`  RÉSUMÉ — BOT MAX V4 (${N} ACTIFS + FILTRE MARCHÉ)`);
  console.log("═══════════════════════════════════════════════════════════════\n");
  ...
  const bullR = results.filter(r => r.regime === "bull");
  const bearR = results.filter(r => r.regime === "bear");
  const neutR = results.filter(r => r.regime === "neutral");

  console.log(`\n  📊 STATS:`);
  console.log(`  ├─ Gagnants: ${winTests}/10`);
  console.log(`  ├─ Meilleur: +${Math.max(...results.map(r => r.pnlPct))}%`);
  console.log(`  ├─ Pire: ${Math.min(...results.map(r => r.pnlPct))}%`);
  console.log(`  ├─ Moyenne: ${avgPnl >= 0 ? '+' : ''}${avgPnl.toFixed(1)}%`);
  console.log(`  ├─ WR moyen: ${avgWR.toFixed(1)}%`);
  console.log(`  ├─ Trades: ${avgTrades.toFixed(0)}`);
  console.log(`  └─ Actifs: ${avgAssets.toFixed(0)}`);

  if (bullR.length > 0) {
    const avg = bullR.reduce((a, b) => a + b.pnlPct, 0) / bullR.length;
    console.log(`  🟢 BULL (${bullR.length}): ${avg >= 0 ? '+' : ''}${avg.toFixed(1)}%`);
  }
  if (bearR.length > 0) {
    const avg = bearR.reduce((a, b) => a + b.pnlPct, 0) / bearR.length;
    console.log(`  🔴 BEAR (${bearR.length}): ${avg >= 0 ? '+' : ''}${avg.toFixed(1)}%`);
  }
  if (neutR.length > 0) {
    const avg = neutR.reduce((a, b) => a + b.pnlPct, 0) / neutR.length;
    console.log(`  🟡 NEUTRAL (${neutR.length}): ${avg >= 0 ? '+' : ''}${avg.toFixed(1)}%`);
  }

  const worst = Math.min(...results.map(r => r.pnlPct));
  console.log(`\n  💡 PIRE SCÉNARIO (${worst}%):`);
  console.log(`  ├─ 1 mois: 250€ → ${(250 * (1 + worst/100)).toFixed(0)}€`);
  console.log(`  └─ 1 an:   250€ → ${(250 * Math.pow(1 + worst/100, 4)).toFixed(0)}€`);
  console.log(`  💡 MOYENNE (${avgPnl >= 0 ? '+' : ''}${avgPnl.toFixed(1)}%):`);
  console.log(`  ├─ 1 mois: 250€ → ${(250 * (1 + avgPnl/100)).toFixed(0)}€`);
  console.log(`  └─ 1 an:   250€ → ${(250 * Math.pow(1 + avgPnl/100, 4)).toFixed(0)}€`);
  console.log("\n═══════════════════════════════════════════════════════════════\n");
}
