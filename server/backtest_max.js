...
function analyze(ana, idx) {
  ...
  if (rsiVal < 40 && isBouncing && volumeOK) {
    return { side: "LONG", entry: price, sl: price - slDistance, tp: price + tpDistance, rr: (tpDistance / slDistance).toFixed(2) };
  }
  if (rsiVal > 60 && isFalling && volumeOK) {
    return { side: "SHORT", entry: price, sl: price + slDistance, tp: price - tpDistance, rr: (tpDistance / slDistance).toFixed(2) };
  }
  return null;
}
...
const ALL_ASSETS = [
  ...
  "MANA-USD", "AXS-USD",
  // ═══ CRYPTO LAYER 1 & 2 (20) ═══
  "HBAR-USD", "XLM-USD", "ALGO-USD", "FLOW-USD", "ICP-USD", "MINA-USD",
  "CELO-USD", "KUSAMA-USD", "DOT-USD", "MATIC-USD", "IMX-USD", "STKADA-USD",
...
async function main() {
  console.log("\n═══════════════════════════════════════════════════════════════");
  console.log(`  BOT MAX — ${ALL_ASSETS.length} ACTIFS | MEILLEURE STRATÉGIE`);
  console.log("═══════════════════════════════════════════════════════════════\n");
  ...
  for (const test of tests) {
    ...
    for (const sym of ALL_ASSETS) {
      try {
        ...
        const result = backtest(sym, data, balance);
    ...
    results.push({ test: test.label, pnlPct: +pnlPct, trades: totalTrades, winRate: +winRate, dd: maxDD, final: balance, assetCount });

    const emoji = +pnlPct >= 0 ? "🟢" : "🔴";
    console.log(`  ${emoji} Test ${test.label}: ${capital}€ → ${balance.toFixed(2)}€ (${pnlPct >= 0 ? '+' : ''}${pnlPct}%) | ${totalTrades} trades | ${winRate}% WR | DD: ${maxDD}% | ${assetCount} actifs actifs`);
  ...
  console.log("\n\n═══════════════════════════════════════════════════════════════");
  console.log(`  RÉSUMÉ — BOT MAX (${ALL_ASSETS.length} ACTIFS)`);
  console.log("═══════════════════════════════════════════════════════════════\n");
  console.log("  Test | Final   | Profit | Trades | Win Rate | DD    | Actifs");
  console.log("  -----|---------|--------|--------|----------|-------|-------");
  for (const r of results) {
    const emoji = r.pnlPct >= 0 ? "🟢" : "🔴";
    console.log(`  ${emoji} ${String(r.test).padEnd(4)} | ${(r.final.toFixed(2) + '€').padEnd(7)} | ${(r.pnlPct >= 0 ? '+' : '') + r.pnlPct + '%'} | ${String(r.trades).padEnd(6)} | ${(r.winRate + '%').padEnd(8)} | ${(r.dd + '%').padEnd(6)} | ${r.assetCount}`);
  }
